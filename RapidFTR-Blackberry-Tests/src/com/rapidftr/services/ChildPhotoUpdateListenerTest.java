...
public class ChildPhotoUpdateListenerTest {
	...
	@Before
	public void setup() {
		store = new ChildrenRecordStore(
				new MockStore(new Key("childrenrecord")));
		callback = mock(RequestCallBack.class);
		createListener(false);
	}

	private void createListener(boolean currentSyncStatus) {
		listener = new ChildPhotoUpdateListener(callback, 1, currentSyncStatus,
				store) {

			protected String savePhoto(Child child, byte[] data)
					throws IOException {
	...
	public void shouldSavePhotoKeyOnSuccessfulResponse() throws Exception {
		...
		Result result = Result.fromContent(
				"{\"current_photo_key\":\"updatedvalue\"}", "application/json");
	...
	@Test
	public void shouldInvokeProcessFailIfOriginalSyncStatusIsFalse()
			throws Exception {
		createListener(true);
		Child child = ChildFactory.existingChild("id");
		Hashtable requestContext = RequestFactory.getRequestContext(child);
